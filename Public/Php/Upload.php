...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (preg_array_key_exists('/ProjectId|LogData|ItemId/', $_POST) && isset($_FILES['file']) && count($_FILES['file'])) {
		...
		if ($ud) {
			...
			$Identifier = substr($_POST['ItemId'], 20);
			...
			foreach ($File_ as $File) if (empty($File['error']) && ! empty($File['tmp_name']) && move_uploaded_file($File['tmp_name'], $Dir . $Identifier . '-' . $File['name'] . '.ogg')) $FileCount ++;
			
			if (count($File_) == $FileCount) {
				
				$DBQ -> prep('UPDATE `u' . $ud['id'] . '_' . $Identifier . '` SET `QResponse` = `QResponse` + ' . $FileCount . ' WHERE `QName` = "RECQTY"');
				
				echo json_encode(array('Action' => 'Upload', 'Successful' => true, 'ItemId' => $_POST['ItemId']));
				
			} else echo json_encode(array('Action' => 'Upload', 'Successful' => false, 'ItemId' => $_POST['ItemId']));
