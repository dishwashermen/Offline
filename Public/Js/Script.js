	...
	if (R) {
		...
		R.upload.onprogress = function(e) {
            ...
            if (e.lengthComputable && SENDFILES) {
                ...
                //document.querySelector('[data-item-id="' + SENDINDEX + '"] #SendFiles').textContent = Progress == 100 ? 'Почти готово...' : Progress + ' %';
	    ...
	    R.upload.onerror = function() {
			...
			//document.querySelector('[data-item-id="' + SENDINDEX + '"] #SendFiles').textContent = 'Отправить';
				...
				if (R.status == 200 && R.responseText.length) {
					...
					switch (RESP.Action) {
						...
						case 'Resume':
							...
							PROJECTLIST[RESP.ItemId].Status = PROJECTLIST[RESP.ItemId].NeedRecord ? 5 : 4;
							
							DB.transaction('Items', 'readwrite').objectStore('Items').put({Id: RESP.ItemId, ProjectId: PROJECT.id, Data: PROJECTLIST[RESP.ItemId]});
							
							document.querySelector('#Abort').click();
							
							// DB.transaction('Media', 'readwrite').objectStore('Media').delete(RESP.ItemId).onsuccess = function(e) {
						...
						case 'Upload':
							...
							if (RESP.Successful) {
								...
								PROJECTLIST[RESP.ItemId].Status = 4;
						...
						case 'Failure': 
							...
							document.querySelector('.item-container[data-item-id="' + SENDINDEX + '"] #Action').classList.remove('preloader');
...
let PageManager = {
	...
	HomePage: function(Page, Reload) {
		...
		if (Object.keys(PROJECTLIST).length) for (let i in PROJECTLIST) {
				
			let ListItem = PROJECTLIST[i];

			let Item = document.querySelector('#ListItem' + (MOBILE ? 'Mobile' : '')).cloneNode(true);
			
			Item.setAttribute('id', i.substring(20));
			
			Item.setAttribute('data-item-id', i);

			Item.querySelector('#ListDate').textContent = GetDate(false, false, ListItem.Date, MOBILE);
			...
			if (PROJECT.Provisional != null) {
				...
				[].forEach.call(PPR, function(P) {
					...
					if (ListItem.hasOwnProperty('Data') && ListItem.Data.hasOwnProperty(P)) {
					
						let Element = new elem('span', {classname: 'span-prov', textcontent: ListItem.Data[P][0]});
				...
				if (! HasProvisional) Item.querySelector('#ListProvisional').classList.add('hidden');
			...
			switch (ListItem.Status) {
				...
				case 1: // Продолжить
					...
					Item.querySelector('#ListProgress').textContent = GetProgress(ListItem.StateIndex);
					
					Item.querySelector('#ListStatus').textContent = 'Продолжить';

					if (MOBILE) {
						
						Item.setAttribute('data-action', 'Resume');
						
						Item.addEventListener('click', listener);
						
					} else  {
						
						Item.querySelector('#Action').classList.add('resume-item');
				
						Item.querySelector('#Action').setAttribute('data-action', 'Resume');
						
						Item.querySelector('#Action').addEventListener('click', listener);
				...
				case 2: // Отправить
					...
					Item.querySelector('#ListStatus').textContent = 'Отправить';
	
					if (MOBILE) {
						
						Item.setAttribute('data-action', 'SendItem');
						
						Item.addEventListener('click', listener);
						
					} else {
						
						Item.querySelector('#Action').classList.add('send-item');
				
						Item.querySelector('#Action').setAttribute('data-action', 'SendItem');
				...
				case 3: // Прервано
					...
					Item.querySelector('#ListStatus').textContent = 'Прервано, не требует отправки';
				
					Item.querySelector('#Action').classList.add('hidden');
				...
				case 4: case 5: // 4- Всё отправлено, 5 - Отправлена только анкета
					...
					Item.querySelector('#ListStatus').textContent = ListItem.Status == 4 ? 'Анкета и аудозапись отправлены' : 'Отправлена только анкета';
					
					if (MOBILE) {
						
						Item.setAttribute('data-action', 'ItemMenu');
						
						Item.addEventListener('click', listener);
						
					} else {
						
						Item.querySelector('#Action').classList.add('menu-item');

						Item.querySelector('#Action').setAttribute('data-action', 'ItemMenu');
...
function builder() {
	...
	if (Object.keys(ITEM.Data).length) for (let i in ITEM.Data) {
		
		let INM = i.replace(/_/g, '.');
		
		let IEL = document.querySelector('[data-out="' + INM + '"]');
		
		if (IEL != null) HistoryDataFill(IEL.type, INM, ITEM.Data[i][0].split(','));
			...
			switch (el.type) {
				...
				case 'file':
					...
					if (el.files.length) {
						...
						let FTT = new elem('table', {});
						
						let FTRT = new elem('tr', {});
						
						FTRT.appendChild(new elem('td', {attr: 'colspan=2', textcontent: GetDate(false, false, PROJECTLIST[el.dataset.itemId].Date, MOBILE)}));
						
						FTT.appendChild(FTRT);
						...
						al('FileList', FTT).then(function() {
							
							if (CONFIRMDATA) SendFiles(el.dataset.itemId);
		...
		case 'click':
			...
			switch (WorkId) {
					...
					if (+ PROJECT.Total > STATEINDEX) {
						...
						if (Object.keys(DATACOMPLETED).length) {
							...
							if (E) { // display error
								...
								if (! NODATA) for (let i in DATACOMPLETED) ITEM.Data[i] = [DATACOMPLETED[i][1].join(), STATEINDEX];
								...
								if (HISTORY) {
									...
									if (STATEINDEX - CurrentState > 1) {
										...
										for (let i in ITEM.Data) {
											...
											if (ITEM.Data[i][1] == STATEINDEX) HISTORYDATA[i] = ITEM.Data[i][0];
											
											if (CleanArray.length) if (CleanArray.includes(ITEM.Data[i][1])) CleanNames.push(i);
											
										}
										
										if (CleanNames.length) for (let i in CleanNames) delete ITEM.Data[CleanNames[i]];
								...
								DB.transaction('Items', 'readwrite').objectStore('Items').put({Id: ITEM.Id, ProjectId: PROJECT.id, Data: ITEM});
				...
				case 'Abort':
					...
					if (Object.keys(PROJECTLIST).length) Cleaner(document.querySelector('#HomePage #ListContainer'));
						...
						if (AuthError === false) {
							...
							if (! PROJECT) {
								...
								DB.transaction('Items').objectStore('Items').index('ProjectId').getAll(ProjectId).onsuccess = function(e) {
									...
									if (e.target.result.length) [].forEach.call(e.target.result, function(x) { PROJECTLIST[x.Id] = x.Data });
				...
				case 'Action':
					...
					let ActionType = el.dataset.action;
					
					if (! el.hasAttribute('data-item-id')) while ((el = el.parentNode) && ! el.hasAttribute('data-item-id'));
					
					switch(ActionType) {
						
						case 'Resume': Action.Get('Current', el.dataset.itemId);
						
						break;
						
						case 'SendItem': SendItem(el.dataset.itemId);
						
						break;
						
						case 'ItemMenu': ItemMenu(el.dataset.itemId);
				...
				case 'Back':
					...
					let IndexInHistory = ITEM.History.indexOf(STATEINDEX);
					
					STATEINDEX = IndexInHistory == -1 ? ITEM.History[ITEM.History.length - 1] : ITEM.History[IndexInHistory - 1];
...
function SendItem(ItemId) {
	...
	if (CANSEND) {
		...
		SendFD.append('ItemId', ItemId);

		SendFD.append('Data', JSON.stringify(PROJECTLIST[ItemId].Data));
		
		let SendButton = document.querySelector('.item-container[data-item-id="' + ItemId + '"] #Action');
		
		if (RECORD && ! PROJECTLIST[ItemId].NeedRecord) {
			
			DB.transaction('Media').objectStore('Media').index('Id').get(ItemId).onsuccess = function(e) {
...
function ItemMenu(ItemId) {
	...
	al('ItemMenu', GetDate(false, false, PROJECTLIST[ItemId].Date, MOBILE)).then(function() {
		...
		if (CONFIRMDATA === 'ext') {
			...
			File.setAttribute('data-item-id', ItemId);
...
function SendFiles(ItemId) {
	...
	if (CANSEND) {
		...
		SendFilesFD.append('ProjectId', PROJECT.id);
		
		SendFilesFD.append('LogData', JSON.stringify(LOGDATA));
		
		SendFilesFD.append('ItemId', ItemId);

		[].forEach.call(document.querySelector('#File').files, function(f) { SendFilesFD.append('file[]', f); });
		
		SENDFILES = true;
		
		SendRequest('Public/Php/Upload.php', SendFilesFD);
...
let Action = {
	...
	Current: function(ItemId) {
		
		ITEM = PROJECTLIST[ItemId];
			
		STATEINDEX = ITEM.StateIndex;
		
		if (! ITEM.NeedRecord && RECORDPERMISSION) {
		
			DB.transaction('Media').objectStore('Media').index('Id').get(ITEM.Id).onsuccess = function(e) {
				
				if (e.target.result) MEDIA = e.target.result.Data;
				
				else MEDIA = new Media();
	...
	Get: function(type, ItemId = false) {
