var dialogData = {
	...
	'ItemMenu': {
		...
		FalseButtonText: 'Выбрать файлы',
		
		ExtButtonText: 'Отправить анкету ещё раз',
	...
	'Closed': {
		...
		QuestionText: 'Проект закрыт. Отправлять анкеты не требуется'
...
class ConfirmDialog {
	...
	_createConfirmDialog() {
		...
		if (typeof(this.QuestionText) == 'object' && this.QuestionText.nodeName != null) {
			
			this.question = elem('div', {classname: 'confirm-dialog-question'});
			
			this.question.appendChild(this.QuestionText);
			
		} else this.question = elem('div', {classname: 'confirm-dialog-question', innerhtml: this.QuestionText});
...
async function al(e, data = false) {
	...
	switch (id) {
		...
		case 'ItemMenu':
			...
			let DialogDataItemMenu = dialogData[id];
			
			DialogDataItemMenu.QuestionText = data;
		
			const ItemMenuDialog = new ConfirmDialog(DialogDataItemMenu);
			
			CONFIRMDATA = await ItemMenuDialog.confirm();
		
		break;
		
		case 'FileList':
		
			let DialogDataFileList = dialogData[id];
			
			DialogDataFileList.QuestionText = data;
		
			const FileListDialog = new ConfirmDialog(DialogDataFileList);
			
			CONFIRMDATA = await FileListDialog.confirm();
		...
		case 'NotSend':
			...
			const NotSendDialog = new ConfirmDialog(dialogData[id]);
			
			await NotSendDialog.confirm();
		
		break;
		
		case 'Closed':
		
			const ClosedDialog = new ConfirmDialog(dialogData[id]);
			
			await ClosedDialog.confirm();
