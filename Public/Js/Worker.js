...
function makeMediaContext() {
    ...
    reader.onload = function(event) {
		...
		DB.transaction('Media', 'readwrite').objectStore('Media').put({Id: PROJECT.id + ':' + LISTINDEX, ProjectId: PROJECT.id, Data: MEDIA});
...
function RecordTimer() {
    ...
    document.querySelector('.top-timer').textContent = 'Идёт запись ' + RECORDTIMER.hour + ':' + (RECORDTIMER.min < 10 ? '0' + RECORDTIMER.min : RECORDTIMER.min) + ':' + (RECORDTIMER.sec < 10 ? '0' + RECORDTIMER.sec : RECORDTIMER.sec);
