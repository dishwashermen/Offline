					...
					switch (RESP.Action) {
						...
						case 'Upload':
							...
							if (RESP.Successful) {
								...
								document.querySelector('#Item' + SENDINDEX + ' #ListOwner').textContent = 'Всё отправлено: ' + PROJECTLIST[RESP.Index].Owner.join(' ');
	...
	HomePage: function(Page, Reload) {
		...
		if (PROJECTLIST.length) {
			...
			[].forEach.call(PROJECTLIST, function(L, i) {
				...
				let Item = document.querySelector('#ListItem').cloneNode(true);
				
				Item.id = 'Item' + (i + 1);
				
				Item.querySelector('#ListId').textContent = (i + 1);
				
				Item.querySelector('#ListDate').textContent = GetDate(false, false, L.Date);
				...
				if (L.NeedRecord) Item.querySelector('#ListNote').textContent = 'Аудиозапись отключена';
				...
				switch (L.Status) {
					...
					case 1: // Продолжить
						...
						Item.querySelector('#Resume').setAttribute('data-index', i);
				
						Item.querySelector('#Resume').addEventListener('click', listener);
						
					break;
					
					case 2: // Отправить
				
						Item.querySelector('#SendItem').setAttribute('data-index', i);
					...
					case 4: case 5: // 4- Всё отправлено, 5 - Отправлена только анкета
						...
						Owner.textContent = (L.Status == 4 ? 'Всё отправлено: ' : 'Отправлена только анкета: ') + L.Owner.join(' ');
					...
					[].forEach.call(RD.ResponseData, function(RDV) {
						...
						switch (RD.Operation) {
							...
							case 'EQUAL': case 'NOT EQUAL':
								...
								Success = (RD.Operation == 'EQUAL' && RDV.QResponse == TX[0]) || (RD.Operation == 'NOT EQUAL' && RDV.QResponse != TX[0]);
								
							break;
							
							case 'MORE': case 'NOT MORE':
							
								Success = (RD.Operation == 'MORE' && RDV.QResponse > TX[0]) || (RD.Operation == 'NOT MORE' && RDV.QResponse <= TX[0]);
								
							break;

							case 'LESS': case 'NOT LESS': 
							
								Success = (RD.Operation == 'LESS' && RDV.QResponse < TX[0]) || (RD.Operation == 'NOT LESS' && RDV.QResponse >= TX[0]);
								
							break;

							case 'CONTAINS': case 'NOT CONTAINS':
							
								let QX = MakeArray(RDV.QResponse);
							...
							case 'BETWEEN': case 'NOT BETWEEN':
								...
								Success = (RD.Operation == 'BETWEEN' && RDV.QResponse >= TX[0] && RDV.QResponse <= TX[1]) || (RD.Operation == 'NOT BETWEEN' && (RDV.QResponse < TX[0] || RDV.QResponse > TX[1]));
			...
			switch (el.id) {
				...
				case 'SendItem':
					...
					if (CANSEND) {
						...
						FD.append('Data', JSON.stringify(PROJECTLIST[el.dataset.index].Data));
						
						if (RECORD && ! PROJECTLIST[el.dataset.index].NeedRecord) {
							
							DB.transaction('Media').objectStore('Media').index('Id').get(PROJECT.id + ':' + el.dataset.index).onsuccess = function(e) {
							
								if (e.target.result) FD.append('Media', JSON.stringify(e.target.result.Data));
